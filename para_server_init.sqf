...
vn_mf_sStartTime = diag_tickTime;
vn_mf_hStartTime = 0;
...
(["GET", "enlisted_counter", 228866] call para_s_fnc_profile_db) params ["",["_enlisted_counter",0]];
...
vn_mf_secondaryTasksBySide setVariable ["TigerForce", []];
vn_mf_secondaryTasksBySide setVariable ["Montagnard", []];
...
// change save time elapsed from saving every 5 seconds to 30 seconds, removed 600 db writes per hour
["save_time_elapsed", {call vn_mf_fnc_save_time_elapsed}, [], 30] call para_g_fnc_scheduler_add_job;
...
["building_state_tracker", {call para_s_fnc_building_state_tracker}, [], 120] call para_g_fnc_scheduler_add_job;
...
//	if(count(_chopped_trees# 0) > 500)then {
//		["SET", "chopped_trees", []] call para_s_fnc_profile_db;
//	};
...
vehicles_nva_planes = [];
// Moved to sites.hpp
vehicles_vc_mortars = ["vn_o_vc_static_mortar_type63", "vn_o_vc_static_mortar_type53", "vn_o_vc_static_d44_01"];
...
// Initialize sites variables
private _sitesCfg = (_gamemode_config >> "sites" >> "sitescfg");
mf_s_max_camps_per_zone = getNumber(_sitesCfg >> "maxcamps");
mf_s_max_aa_per_zone = getNumber(_sitesCfg >> "maxaa");
mf_s_max_artillery_per_zone = getNumber(_sitesCfg >> "maxarty");
mf_s_max_fortifications_per_zone = getNumber(_sitesCfg >> "maxfortification");
mf_s_max_tunnels_per_zone = getNumber(_sitesCfg >> "maxtunnels");
mf_s_max_roadblocks_per_zone = getNumber(_sitesCfg >> "maxroadblocks");
mf_s_max_tunnels_watersupply_per_zone = getNumber(_sitesCfg >> "maxtunnelwatersupply");
mf_s_max_vehicles_per_zone = getNumber(_sitesCfg >> "maxvehicles");
mf_s_max_helicopters_per_zone = getNumber(_sitesCfg >> "maxhelicopters");
mf_s_loaded_sites = [];

diag_log "VN MikeForce: Initialising Dac Cong Respawn";
DCRespawnCounter = 0;
DCRespawnPoints = [];


localNamespace setVariable ["sites", []];
localNamespace setVariable ["sites_hq", []];
...
[] call para_s_fnc_cleanup_arsenal_init;

diag_log "VN MikeForce: Initialising BroNation Player Rewards";
