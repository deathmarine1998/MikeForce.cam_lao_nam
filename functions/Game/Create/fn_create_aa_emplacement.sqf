...
private _worldCfg = format["aaemplacements_%1",toLower(worldName)];
private _aaCfg = getArray(_sitesCfg >> _worldCfg);
if((_aaCfg isEqualTo []) || !(_aaCfg isEqualType []))then {
	_aaCfg = ["vn_o_nva_static_dshkm_high_02", "vn_o_nva_static_zpu4"];
};

private _aaGun = _aaCfg select (_style == "heavy");
