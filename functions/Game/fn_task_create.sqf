...
if (isClass _taskConfig) then
{
	...
	if(_debug isEqualTo 1) then { ["_isAllowed %1 _targetGroups %2 taskClass %3 _taskgroupID %4 _requestgroups %5",_isAllowed,_targetGroups, _taskClass, _taskgroupID, _requestgroups] call BIS_fnc_logFormat; };
	...
	if (_isAllowed) exitWith
	{
		...
		[_targetGroups, _taskFrameworkId, [_formattedTaskDesc,_formattedTaskName,_taskmarker], objNull, "CREATED", 100, false, _tasktype, true] call BIS_fnc_taskCreate;
		...
		if (_taskCategory == "SUP") then {
			["NewSupportRequest", ["", _formattedTaskName]] remoteExec ["para_c_fnc_show_notification", _targetGroups];
		};
