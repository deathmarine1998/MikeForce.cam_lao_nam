...
private _fnc_considerAttack = {
	...
	if ((_infantryStrength > 0) && (_infantryStrength <= 0.3) && (count (allPlayers inAreaArray _zoneMarker) > 0)) then {
		...
		private _zones = format["zones_%1",worldName];
		private _connectedZones = getArray (missionConfigFile >> >> "map_config" >> _zones >> _zoneMarker) apply {_x select 0};
