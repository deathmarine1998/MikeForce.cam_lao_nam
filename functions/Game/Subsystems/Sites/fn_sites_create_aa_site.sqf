...
private _aaGrid = getNumber(_config >> "aagrid");
private _aaIcon = getNumber(_config >> "aaicon");
private _debug = getNumber(missionConfigFile >> "gamemode" >> "debug" >> "aimarkers");

private _aaGridAlpha = 0.2;
if(_aaGrid isEqualTo 0)then { _aaGridAlpha=0 };
...
[
	...
	//Setup Code
	{
		...
		_aaZoneMarker setMarkerAlpha _aaGridAlpha; //0.3;
		...
		_aaMarker setMarkerAlpha _aaIconAlpha; //tried 0.3 too light returned to default 
