...
private _debug = getNumber(_gamemodeConfig >> "debug" >> "vehicles");
private _lockingSystem = getNumber(_gamemodeConfig >> "debug" >> "lockingSystem");
...
private _fnc_processVehicle = {
	...
	if (!alive _vehicle && !((call _fnc_getState select _state) in ["RESPAWNING", "REPAIRING", "WRECKED"])) then {
		...
		if (_respawnType == "IDLERESPAWN") exitWith {
			[_id, _respawnTime] call vn_mf_fnc_veh_asset_set_respawning;
		};
	};
	...
	if ((call _fnc_getState select _state) == "IDLE") then {
		...
		if (
			...
			if(_respawnType == "IDLERESPAWN")then { deleteVehicle _vehicle };
		...
		if (
			...
			// slung load check
			!(_vehicle getVariable ["ASL_SlungLoad", false]) &&
			...
			if(_lockingSystem isEqualTo 1) then { [_id] call vn_mf_fnc_veh_asset_lock_vehicle };
		};
	};
