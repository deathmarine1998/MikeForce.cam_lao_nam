...
_taskDataStore setVariable ["INIT", {
	...
	_taskDataStore setVariable ["buildCopTasks", _buildTasks];

	[[
		["destroy_camps", _zonePosition],
		["build_fob", _zonePosition],
		["build_cop", _zonePosition]
...
_taskDataStore setVariable ["build_cop", {
	params ["_taskDataStore"];

	private _buildCopTasks = _taskDataStore getVariable "buildCopTasks";
	private _buildCopTasksCompleted = {_x call vn_mf_fnc_task_is_completed} count _buildCopTasks;
	if (count _buildCopTasks == _buildCopTasksCompleted) then {
		["SUCCEEDED"] call _fnc_finishSubtask;
	};
}];
