...
_taskDataStore setVariable ["defend_base", {
	...
	if (serverTime - _startTime > (_taskDataStore getVariable ["holdDuration", 2400]) 
...
_taskDataStore setVariable ["AFTER_STATES_RUN", {
	...
	if (_taskDataStore getVariable ["areaDefended", false] || mf_s_cia_defend_fob) then {
