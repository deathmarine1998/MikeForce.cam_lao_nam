...
addLocations = {
	_townMarkerArray = [];
	{
		if (["zone_", _x] call fn_startsWith) then
		{
...
private _keyDown = preprocessFile format["eventhandlers\display\eh_%1.sqf","KeyDown"];
_spectatePress = (findDisplay 60492) displayAddEventHandler ["KeyDown", _keyDown];
private _keyUp = preprocessFile format["eventhandlers\display\eh_%1.sqf","KeyUp"];
_spectateRelease = (findDisplay 60492) displayAddEventHandler ["KeyUp", _keyUp];
*/
false