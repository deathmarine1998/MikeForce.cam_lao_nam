...
_group_data = getArray(MissionConfigFile >> "gamemode" >> "settings" >> "teams" >> _groupID);// params[["_group_name_full", "ERROR: GROUP NAME FULL"], ["_group_icon", "Vn\ui_f_vietnam\ui\debrief\sticky.paa"], ["_group_name", "ERROR: GROUP NAME"]];
VN_TR_MAININFO_IMG_CTRL ctrlSetText _group_data# 1;
_group_name_bold = format["<t align='center' font='tt2020base_vn_bold'>%1</t>", _group_data# 2];
...
private _desc = "";
_desc = format["STR_vn_mf_taskRoster_Main_%1Description",_groupID];
_text = _desc;

VN_TR_MAININFO_TXT_MID_CTRL ctrlSetStructuredText parseText format [localize _text, format ["<img size='5' color='#FFFFFF' image='%1'/>", _group_data# 1]];
...
if(_groupID == "DacCong")then {
	VN_TR_CCDC_BTN_CTRL ctrlEnable true;
	VN_TR_CCDCOFF_BTN_CTRL ctrlEnable true;

	VN_TR_CCMF_BTN_CTRL ctrlEnable false;
	VN_TR_CCMFOFF_BTN_CTRL ctrlEnable false;
	VN_TR_CCAC_BTN_CTRL ctrlEnable false;
	VN_TR_CCACOFF_BTN_CTRL ctrlEnable false;
	VN_TR_CCGH_BTN_CTRL ctrlEnable false;
	VN_TR_CCGHOFF_BTN_CTRL ctrlEnable false;
	VN_TR_CCST_BTN_CTRL ctrlEnable false;
	VN_TR_CCSTOFF_BTN_CTRL ctrlEnable false;
};