...
private _accepted_groups = getArray(missionConfigFile >> "gamemode" >> "settings" >> "groups" ) apply {_x select 0};
if !(_groupID in _accepted_groups)then { _groupID = "FAILED"};

systemchat str ["_groupID: ",_groupID];
private _group_data = getArray(MissionConfigFile >> "gamemode" >> "settings" >> "teams" >> _groupID);
//set name and logo of the Team, the player is in
[VN_TR_TEAMNAME_CTRL, (_group_data#0)] call vn_fnc_UI_checkFontSize;
VN_TR_TEAMNAME_CTRL ctrlSetText _group_data#0;
VN_TR_TEAMLOGO_CTRL ctrlSetText _group_data#1;
