...
class vn_tr_disp_taskRoster_Main
{
	...
	class Controls
	{
		...
		class team_logo_btn: vn_mf_RscButton
		{
			...
			tooltip = "";
			text = "";
			onButtonClick = "systemchat str [groupID (group player)];if!((player getVariable 'vn_mf_db_player_group') isEqualTo 'DacCong')then { call vn_mf_fnc_tr_cleanRightSheet; createDialog 'vn_tr_disp_selectTeam' };";
		};
		
		class cia_logo: vn_mf_RscPicture
		{
			idc = VN_TR_CIALOGO_IDC;
		...
		class cia_btn: vn_mf_RscButton
		{
			idc = VN_TR_CIA_BTN_IDC;
			x = UIX_CL(10);
			y = UIY_CU(10);
			w = UIW(1);
			h = UIH(1);
