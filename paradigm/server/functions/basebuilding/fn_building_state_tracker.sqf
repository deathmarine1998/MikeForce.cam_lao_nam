...
private _dcRespawnObj = getText(missionConfigFile >> "gamemode" >> "settings" >> "dcrespawns" >> "buildingtype");
{
	private _building = _x;
	if(typeOf _building == _dcRespawnObj)exitWith {};
	...
	private _placedTime = _building getVariable ["para_g_build_placedTime",0];
	private _buildProgress = _building getVariable ["para_g_build_progress",0];
	private _sincePlaced = serverTime - _placedTime;
	//If somehow we've no longer got any objects, building is invalid, delete it.
	if( (_objects isEqualTo []) || ((_sincePlaced > 150) && (_buildProgress == 0)) )then {
