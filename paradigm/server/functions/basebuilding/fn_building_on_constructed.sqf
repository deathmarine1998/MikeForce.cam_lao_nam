...
private _copsContaining = getPos _building call para_g_fnc_cops_containing_pos;
private _isCopStarter = isClass (_buildableConfig >> "features" >> "cop_starter");
if (_isCopStarter && _copsContaining isEqualTo []) then {
	private _cop = [getPos _building] call para_s_fnc_cop_create;
	_copsContaining pushBack _cop;
};
if !(_copsContaining isEqualTo []) then {
	[_building, _copsContaining # 0] call para_s_fnc_building_connect_cop;
};
...
private _isFobFlag = isClass (_buildableConfig >> "features" >> "flag_starter");
if (_isFobFlag && !mf_s_baseFlagBuilt) then {
	mf_s_baseFlagBuilt = true;
};
