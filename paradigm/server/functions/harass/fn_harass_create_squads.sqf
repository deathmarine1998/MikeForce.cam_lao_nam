...
{
	private _side = _x;
	private _enemySides = [west, independent, civilian] select { _x getFriend _side < 0.6 }; // REMOVED side east,
	...
	private _harassablePlayers = _friendlyPlayers select {
		...
		//Player is on a combat side. // REMOVED side east,
		&& side group _player in [west, independent]
	};
	...
	{
		if(_debug isEqualTo 1) then { ["Player: %1, Harass Level: %2", _x, _x getVariable "harass_level"] call BIS_fnc_logFormat };
	} forEach _harassablePlayers;
	...
	while {!(_friendlyPlayersToHarass isEqualTo []) && count _friendlyPlayersToHarass != _lastPlayersToHarassLength} do {
		...
		if (_sufficientTimeHasPassed && _averageHarassment <= 0.5) then {
			...
			if(_debug isEqualTo 1) then { ["Requesting pursuit with %1 unit scaling", [count _nearbyPlayers, _challengeDifference]] call BIS_fnc_logFormat };
			private _enemySide = [_side, getPos _target] call para_interop_fnc_harass_get_enemy_side;
			[_nearbyPlayers, _challengeDifference, _enemySide] call para_s_fnc_ai_obj_request_pursuit;
...
} forEach [west, independent]; // REMOVED side east,

if(_debug isEqualTo 1) then { ["Finished harassing players"] call BIS_fnc_logFormat };
