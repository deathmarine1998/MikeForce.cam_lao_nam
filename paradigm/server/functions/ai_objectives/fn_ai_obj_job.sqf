...
private _allPlayers = allUnits select {(side _x == west || side _x == independent) && !(vehicle _x isKindOf "Plane") && !(speed vehicle _x > 300)};
private _debug = getNumber(missionConfigFile >> "gamemode" >> "debug" >> "mikeforceAIlogging");
...
if(_debug isEqualTo 1) then { diag_log "AI Obj: Ticking Objectives" };
{
...
if(_debug isEqualTo 1) then { diag_log format ["AI Obj: Activating Objectives: %1", _newlyActiveObjectives apply {_x getVariable "id"}] };
if(_debug isEqualTo 1) then { diag_log format ["AI Obj: Deactivating Objectives: %1", _deactivatedObjectives apply {_x getVariable "id"}] };
if(_debug isEqualTo 1) then { diag_log format ["AI Obj: Active Objectives: %1", _activeObjectives apply {_x getVariable "id"}] };
...
private _poolSize = para_s_ai_obj_hard_ai_limit;
private _currentUnitCount = count _enemyUnits;

if(_debug isEqualTo 1) then { diag_log format ["AI Obj: Current Pool Size: %1, Count: %2", _poolSize, _currentUnitCount] };
...
if(_debug isEqualTo 1) then { diag_log format ["AI Obj: Prioritised Objectives: %1", _objectivesPrioritisedHighestFirst apply {_x getVariable "id"}] };
...
//Now we go in order of highest priority, and allocate units where needed.
{
	...
	private _availableReinforcements = 
		_obj getVariable ["reinforcement_pools", []] 
		apply 
		{
			[[_x] call para_s_fnc_ai_obj_available_reinforcements, _x]
		} 
		select {_x # 0 > 0};
	...
	if(_debug isEqualTo 1) then {
		diag_log format [
			"AI Obj %1: Players: %2, Squad Size: %3, Desired: %4, Current Alive: %5, Reinforcements: %6", 
			_obj getVariable "id",
			_playerCount,
			_squadSize,
			_desiredUnitCount,
			_totalAliveUnits,
			str (_availableReinforcements apply { [_x # 0, getPos (_x # 1)] })
		];
	};
	...
	call 
	{
		...
		if (_availableReinforcements isEqualTo []) exitWith {};

		//Improve this, so it doesn't over-allocate reinforcements.
		//Right now, over-allocating means if we want 30 guys, and the pool has 1 left, we get 30 guys.
		//Might make for a more interesting player experience at least - not necessarily a bad thing!
		//Could loop through, and only allocate what we have available, for example.
		//But is it actually worth it?
		private _reinforcementsToUse = _availableReinforcements # 0;
		private _reinforcementsPool = _reinforcementsToUse # 1;

		//Still need more men - let's get some fresh ones.
		private _unitsRemainingInPool = _poolSize - _currentUnitCount;
		private _unitsToSendCount = 
			_reinforcementsNeeded 
			min 
			_unitsRemainingInPool
			max 0;

		if(_debug isEqualTo 1) then {
			diag_log format [
				"AI Obj %1: Desired Units: %2, Sending: %3",
				_obj getVariable "id",
				_desiredUnitCount,
				_unitsToSendCount
			];
		};
		[_obj, _unitsToSendCount, _reinforcementsPool] call para_s_fnc_ai_obj_reinforce;
