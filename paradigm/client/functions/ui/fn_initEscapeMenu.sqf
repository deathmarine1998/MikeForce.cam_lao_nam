/*
	File: fn_ui_initEscapeMenu.sqf
	Author: Terra
	Date: 2020-06-12
	Last Update: 2020-06-12
	Public: No
	
	Description:
		Automatically adds buttosn to the escape menu which opens the keybindings menu. Called from CfgFunctions with postInit = 1.
	
	Parameter(s): none
	
	Returns: nothing
	
	Example(s): none
*/

#include "..\..\configs\ui\ui_def_base.inc"

[missionNamespace, "OnGameInterrupt", {
	params ["_display"];

	// Bro Nation Button
	_bnoBtn = _display ctrlCreate ["para_RscButtonMenu", -1];
	_bnoBtn ctrlSetPosition [safeZoneX + UIW(1), safeZoneY + UIH(1), UIW(15), UIH(1)];
	_bnoBtn ctrlCommit 0;
	_bnoBtn ctrlSetText "BRO NATION OPERATIONS";
	_bnoBtn ctrlAddEventHandler ["ButtonClick",{
		params ["_btn"];
		_escDisplay = ctrlParent _btn;
		["BroNationMsg",["Thank you for enjoying the community!"]] call para_c_fnc_show_notification;
	}];

	// Keybindings
	_newBtn = _display ctrlCreate ["para_RscButtonMenu", -1];
	_newBtn ctrlSetPosition [safeZoneX + UIW(1), safeZoneY + UIH(2.1), UIW(15), UIH(1)];
	_newBtn ctrlCommit 0;
	_newBtn ctrlSetText "GAMEMODE KEYBINDINGS";
	_newBtn ctrlAddEventHandler ["ButtonClick",{
		params ["_btn"];
		_escDisplay = ctrlParent _btn;
		_escDisplay createDisplay "para_RscDisplayKeybindingsMenu";
	}];

    // Options
	_optsBtn = _display ctrlCreate ["para_RscButtonMenu", -1];
	_optsBtn ctrlSetPosition [safeZoneX + UIW(1), safeZoneY + UIH(3.2), UIW(15), UIH(1)];
	_optsBtn ctrlCommit 0;
	_optsBtn ctrlSetText "GAMEMODE OPTIONS";
	_optsBtn ctrlAddEventHandler ["ButtonClick",{
		params ["_btn"];
		_escDisplay = ctrlParent _btn;
		[_escDisplay] call para_c_fnc_optionsMenu_init;
	}];

	_startBtn = _display ctrlCreate ["para_RscButtonMenu", -1];
	_startBtn ctrlSetPosition [safeZoneX + UIW(1), safeZoneY + UIH(4.3), UIW(15), UIH(1)];
	_startBtn ctrlCommit 0;
	_startBtn ctrlSetText "WELCOME MENU";
	_startBtn ctrlAddEventHandler ["ButtonClick",{
		params ["_btn"];
		createDialog "para_WelcomeScreen";
	}];

    // Bug Report
	if!(groupID group player in["MikeForce","ACAV","GreenHornets","SpikeTeam"])then {
		_bugBtn = _display ctrlCreate ["para_RscButtonMenu", -1];
		_bugBtn ctrlSetPosition [safeZoneX + UIW(1), safeZoneY + UIH(5.4), UIW(15), UIH(1)];
		_bugBtn ctrlCommit 0;
		_bugBtn ctrlSetText "BUGS/SUGGESTIONS";
		_bugBtn ctrlAddEventHandler ["ButtonClick",{
			params ["_btn"];
			_escDisplay = ctrlParent _btn;
			[_escDisplay] call para_c_fnc_bugReport_init;
		}];
	};

}] call BIS_fnc_addScriptedEventHandler;