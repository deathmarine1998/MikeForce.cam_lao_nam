...
if([_hitObject] call para_g_fnc_is_valid_axe_explosive_target)exitWith {
	if(random(100) < 10)then {
		[_hitObject, player] remoteExecCall ["vn_mf_fnc_server_destroy_by_axe", 2];
	};
};
if (!([_hitObject] call para_g_fnc_is_valid_axe_target)) exitWith {};
if(typeOf _hitObject == "Land_vn_t_piceaabiesnativitatis_2s")exitWith {
...
{
    ["fell_tree", [_hitObject, _currentPos, player]] call para_c_fnc_call_on_server;
    player setVariable ["para_c_axe_progress", nil];
    player setVariable ["para_c_axe_progress_tree", nil];
};
