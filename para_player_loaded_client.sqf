...
[true, "arsenalClosed", {
	[player, "NoVoice"] remoteExec ["setSpeaker", 0];
	if("vn_m72" in weapons player)then { player addMagazine "vn_m72_mag" };	
...
private _worldCfg = format["images_%1",toLower(worldName)];
[selectRandom (getArray(missionConfigFile >> "gamemode" >> "loadingScreens" >> _worldCfg)),5002] call vn_mf_fnc_update_loading_screen;